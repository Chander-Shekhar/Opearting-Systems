...
bool PeriodCompare(process &a , process &b)
{
	return (a.period < b.period);
}

void Rate_monotonic(process *tasks, int n, long long max_time)
{
	ofstream output2_file;
	output2_file.open("RM-Stats.txt");
	...
	sort(tasks, tasks + n, PeriodCompare);
	while(time <= max_time)
	{
		curr_process = -1;
		for(i=0;i<n;i++)
...
int main()
{
	...
	Rate_monotonic(tasks,n,max_time);
